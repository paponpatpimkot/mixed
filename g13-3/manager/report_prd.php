  ...
  <center><div class="print"><input type="button" value="พิมพ์" onclick="window.print();" /> 
    <?php
		if(isset($_GET['report'])){
			...
			echo "<a href='report_year.php'><button>ไม่พิมพ์</button></a>";
		}
	?></div></center>
    <br />
...
<center>
  ...
  ประจำปี <?php echo $_GET[year]+543; ?>
    </center><br />
    </fieldset>
    <fieldset>
    ...
    <?php
		...
		$total = 0;
		if(isset($_GET['date'])){
			$select_sell = $con->query("select *,sum(selling_list.sell_num) as num,sum(selling_list.sell_price) as price from selling,selling_list,product where selling.sell_id = selling_list.sell_id and selling_list.pro_id = product.pro_id and sell_date = '$_GET[date]' group by selling_list.pro_id having sum(selling_list.sell_price) order by $_GET[order] desc");
		}elseif(isset($_GET['month'])){
