...
<body onload="document.fs.search.focus();">
		...
		?>
        <div class="top"></div>
        <div class="headcon"><h4>จัดการข้อมูลหน่วยสินค้า</h4></div>
        <div class="content"><br>

        	<div class="rnow"><a class="button" href="add_unit.php">+ เพิ่มข้อมูลหน่วยสินค้า</a></div><br>
					<div class="wrap">
            ...
            <div>
            	<table class="responstable">
                    ...
                    <?php
										...
										$row = 10;
										if($_GET['search']!=""){
											$total_data = mysqli_num_rows($con->query("select * from unit where unit_name LIKE '%$_GET[search]%'"));
											...
											$start = ($page-1)*$row;
											$sql = $con->query("select * from unit where unit_name LIKE '%$_GET[search]%' order by unit_name  limit $start,$row");
										}else{
										$total_data = mysqli_num_rows($con->query("select * from unit"));
										$total_page = ceil($total_data/$row);
										$page = @$_GET['page'];
										if($page<1){
											$page=1;
										...
										$start = ($page-1)*$row;
										$sql = $con->query("select * from unit order by unit_name limit $start,$row");
										}
                    ...
                    <tr>
                        ...
                        <td align="center"><a class="buttonee" href="edit_unit.php?id=<?php echo $r['unit_id']; ?>">แก้ไข</a></td>
                    </tr>
                    <?php } ?>
                </table>
